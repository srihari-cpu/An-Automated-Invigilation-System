...
//   document.getElementById("selectedFaculty").innerText = `Selected Faculty: ${selectedFaculties.join(", ")}`;
...
//   const selectedFaculties = document.getElementById("facultyInput").value.split(", ").filter((name) => name !== "");
//   const hallInput = document.getElementById("hallNames").value;
//   const halls = hallInput.split(",").map((name) => name.trim()).filter((name) => name !== "");
...
//   if (!examDate || selectedFaculties.length === 0 || halls.length === 0 || selectedSessions.length === 0) {
...
//     halls.forEach((hall) => {
//       const assignedFaculty = selectedFaculties.length > 0
//         ? selectedFaculties.splice(Math.floor(Math.random() * selectedFaculties.length), 1)[0]
//         : "No Faculty";

//       const floorNumber = hall.charAt(0);
//       assignments.push({
//         date: examDate,
//         session: session,
//         block: blockName,
